...
`/events` is a page where use can reserve time slot for certain event:
...
Burp Suite shows multiple js files and one of them reveals the version info for booking press: **ver=1.0.10**
...
Using the extracted nonce, I modified the poc command and querying for version, version_comment, and version_compile_os confirms SQL injection working:
...
| ---------- | -------------- | ------------------------- |
| blog       | blog@localhost | information_schema, blog  |
